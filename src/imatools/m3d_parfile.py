...
def error_in_paths(pot: dict):
    mylist = [
        pot["segmentation"]["seg_dir"],
        pot["segmentation"]["seg_name"],
        pot["output"]["outdir"],
        pot["output"]["name"],
    ]
...
if __name__ == "__main__":
    ...
    iparse.add_argument(
        "--filepath", "-f", type=str, required=True, help="Output Meshtools3d parameter file"
    )
    iparse.add_argument(
        "--template", "-t", type=str, help="Path to template parameter file (JSON or .par)"
    )
    iparse.add_argument(
        "--json", "-json", action="store_true", help="If set, saves output as a JSON file"
    )
    ...
    paramters_group.add_argument(
        "--mesh_from_segmentation", "-mesh_from_segmentation", type=bool, metavar="1/0"
    )
    paramters_group.add_argument(
        "--boundary_relabeling", "-boundary_relabeling", choices=[0, 1], type=int, metavar="1/0"
    )
    ...
    paramters_group.add_argument(
        "--facet_distance", "-facet_distance", type=float, help="Facet distance"
    )
    paramters_group.add_argument(
        "--cell_rad_edge_ratio", "-cell_rad_edge_ratio", type=float, help="Cell radius edge ratio"
    )
    paramters_group.add_argument("--cell_size", "-cell_size", type=float, help="Cell size")
    paramters_group.add_argument(
        "--rescale_factor", "-rescaleFactor", type=float, help="Rescale factor"
    )
    ...
    paramters_group.add_argument(
        "--dimKrilovSp", "-dimKrilovSp", type=int, help="Dimension of the Krylov space"
    )
    paramters_group.add_argument("--verbose", "-verbose", choices=[0, 1], type=int)
    paramters_group.add_argument(
        "--eval_thickness", "-eval_thickness", choices=[0, 1], type=int, metavar="1/0"
    )
    paramters_group.add_argument(
        "--outdir",
        "-outdir",
        type=str,
        default=os.getcwd(),
        help="Subdirectory of outputs in parfile",
    )
    paramters_group.add_argument(
        "--name", "-name", type=str, default="myocardium", help="Name of the output file"
    )
    paramters_group.add_argument(
        "--out_medit", "-out_medit", choices=[0, 1], type=int, metavar="1/0"
    )
    ...
    paramters_group.add_argument(
        "--out_vtk_binary", "-out_vtk_binary", choices=[0, 1], type=int, metavar="1/0"
    )
    paramters_group.add_argument(
        "--out_carp_binary", "-out_carp_binary", choices=[0, 1], type=int, metavar="1/0"
    )
    paramters_group.add_argument(
        "--out_potential", "-out_potential", choices=[0, 1], type=int, metavar="1/0"
    )
