...
_IGNORE_ERRORS = ["unordered_map::at()"]
...
def execute_scale_mesh(input_info: dict, scale: float = 0.001, convert_format: bool = False) -> str:
    ...
    # meshtool args are '='-joined single tokens — safe as-is for no-shell execution.
...
def execute_deform_mesh(
    ...
    # Step 1: Rigid registration. Flatten space-joined flags into separate
    # tokens — pycemrg runs without a shell, so each token must stand alone.
    arguments = [
        path_to_moving,
        path_to_fixed,
        "-dofout",
    ...
    cmd = [register_exe, *arguments]
    logger.info(f'Executing registration: {" ".join(cmd)}')
    cmd_runner.run(cmd, expected_outputs=[dof_file], ignore_errors=_IGNORE_ERRORS)
    ...
    arguments = [input_mesh_path, output_mesh_path, "-dofin", dof_file]
    cmd = [transform_exe, *arguments]
    logger.info(f'Executing transformation: {" ".join(cmd)}')
    cmd_runner.run(cmd, expected_outputs=[output_mesh_path], ignore_errors=_IGNORE_ERRORS)
...
def validate_pipeline_args(args) -> None:
    ...
    missing = [desc for arg, desc in required_for_pipeline.items() if getattr(args, arg) is None]
...
def update_arguments_for_cwd(args, input_info) -> None:
    ...
    if args.infer_working_directory:
        ...
        if args.path_to_moving:
            args.path_to_moving = os.path.join(
                input_info["dirname"], os.path.basename(args.path_to_moving)
            )
        if args.path_to_fixed:
            args.path_to_fixed = os.path.join(
                input_info["dirname"], os.path.basename(args.path_to_fixed)
            )
        if args.reference_image:
            args.reference_image = os.path.join(
                input_info["dirname"], os.path.basename(args.reference_image)
            )
    else:
...
def main(args):
    ...
    try:
        if mode == "pipeline":
            ...
            execute_scale_mesh(input_info, scale=args.scale, convert_format=args.convert_format)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Project Ventricular Scar from CINE", usage=main.__doc__, add_help=False
    )

    parser.add_argument(
        "mode",
        type=str,
        choices=["pipeline", "scale", "deform", "cog", "scar"],
    ...
    parser.add_argument(
        ...
        "-cwd",
        action="store_true",
        help="Infer working directory from input file path",
    )
    ...
    scale_group.add_argument(
        ...
        "-format",
        action="store_true",
        help="Convert mesh format (vtk <-> carp_txt)",
    )

    # Deform options
    deform_group = parser.add_argument_group("Deform Options")
    deform_group.add_argument(
        "--path-to-mirtk", "-mirtk", type=str, help="Path to MIRTK executables folder"
    )
    deform_group.add_argument(
        "--path-to-moving", "-moving", type=str, help="Path to moving image (CINE)"
    )
    deform_group.add_argument(
        "--path-to-fixed", "-fixed", type=str, help="Path to fixed image (LGE)"
    )

    # Scar projection options
    scar_group = parser.add_argument_group("Scar Projection Options")
    scar_group.add_argument(
        "--reference-image", "-ref", type=str, help="Path to reference image (LGE Segmentation)"
    )
    scar_group.add_argument(
        "--label", "-label", type=int, default=3, help="Label for scar region (default: 3)"
    )
