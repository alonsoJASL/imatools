...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parent_parser.add_argument(
        "-out", "--output", type=Path, default=None, help="Output image path (optional)"
    )
    parent_parser.add_argument(
    ...
    pr = sub.add_parser(
        ...
        help="Collapse foreground labels in an image to a single value",
    ...
    pm = sub.add_parser(
        "morph-label", help="Morph one label, mask back into image", parents=[parent_parser]
    )
    _add_label(pm)
    ...
    pmsk = sub.add_parser(
        "mask",
        help="Masks a label map image with a binary mask (defaults to 0)",
        parents=[parent_parser],
    )
    ...
    pmsk.add_argument(
        ...
        type=int,
        default=0,
        help="Value to write into the masked region",
    )
    pmsk.add_argument(
        "-ignore", "--ignore-image", type=Path, default=None, help="Path to ignore image (optional)"
    )
    ...
    pdel = sub.add_parser(
        ...
        help="Delete one or more labels from an image",
        parents=[parent_parser],
    )
    ...
    pmrg = sub.add_parser(
        "merge-labels",
        aliases=["merge"],
        help="Merge multiple labels into one",
        parents=[parent_parser],
    )
    _add_label(pmrg, multiple=True)
    pmrg.set_defaults(func=handle_merge_labels)

    psplt = sub.add_parser(
        "split-label",
        aliases=["split", "cc-split"],
        help="Split a label into its connected components",
        parents=[parent_parser],
    )
    _add_label(psplt)
    psplt.add_argument(
        "--open-image",
        action="store_true",
        help="Preprocess with morphological opening to remove small connections",
    )
    psplt.add_argument(
        "--open-radius", type=int, default=3, help="Radius for morphological opening preprocessing"
    )
    ...
    padd.add_argument(
        "-in2", "--secondary-image", type=Path, required=True, help="Second image to add"
    )
    padd.set_defaults(func=handle_add)

    pop = sub.add_parser(
        "op", help="Voxelwise operation between two images", parents=[parent_parser]
    )
    ...
    pfill = sub.add_parser(
        "fill", help="Fill gaps in a (multi)label segmentation", parents=[parent_parser]
    )
    pfill.add_argument(
        "-in2",
        "--secondary-image",
        type=Path,
        default=None,
        help="Optional previous segmentation to fill from",
    )
    pfill.set_defaults(func=handle_fill)

    plrg = sub.add_parser(
        "largest", help="Keep only the largest connected component", parents=[parent_parser]
    )
    plrg.set_defaults(func=handle_largest)

    pgap = sub.add_parser(
        "gaps", help="Show gaps in a (multi)label segmentation", parents=[parent_parser]
    )
    pgap.add_argument(
        "--binary", action="store_true", help="Treat input as already binary (default: multilabel)"
    )
    ...
    psw = sub.add_parser(
        "swap",
        aliases=["swap-labels"],
        help="Swap labels old->new (lists and/or an inclusive range)",
        parents=[parent_parser],
    )
    ...
    psw.add_argument(
        ...
        nargs="+",
        default=[],
        help="Replacement labels (1:1 with --old-labels)",
    )
    psw.add_argument(
        "--old-label-range",
        type=str,
        default="",
        help="Inclusive range 'a:b' appended to --old-labels",
    )
    psw.add_argument(
        "--range-replace",
        type=int,
        default=None,
        help="Single value to map the whole --old-label-range to",
    )
    ...
    prs = sub.add_parser(
        "resample",
        aliases=["smooth"],
        help="Resample/smooth a label image",
        parents=[parent_parser],
    )
    prs.add_argument(
        "--spacing", type=float, nargs=3, default=[1.0, 1.0, 1.0], help="Target spacing"
    )
    ...
    pwcomp.add_argument(
        "-in2", "--secondary-image", type=Path, required=True, help="Second image to compare"
    )
    pwcomp.add_argument(
        "--swap-axes", action="store_true", help="Swap axes of the second image before comparison"
    )
    pwcomp.add_argument(
        "--save-to-report", action="store_true", help="Save comparison results to a report file"
    )
    pwcomp.set_defaults(func=handle_compare_label_maps)

    pcmb = sub.add_parser(
        "combine",
        help="Combine all binary segmentations in a FOLDER (-in) into one label image",
        parents=[parent_parser],
    )
    pcmb.add_argument(
        "--pattern", default="*.nrrd", help="Glob for images in the input folder (default: *.nrrd)"
    )
    pcmb.set_defaults(func=handle_combine)

    pchain = sub.add_parser(
        "chain",
        help="Apply a sequence of single-label morphological ops in order",
        parents=[parent_parser],
    )
    pchain.add_argument(
        "--operations",
        nargs="+",
        required=True,
        help="Operations to apply in order (open/close/dilate/erode/fillholes)",
    )
    pchain.add_argument(
        "--labels",
        type=int,
        nargs="+",
        required=True,
        help="Label for each operation (1:1 with --operations)",
    )
    pchain.add_argument(
        "--radii",
        type=int,
        nargs="+",
        default=[],
        help="Radius per operation; if omitted, --radius is used for all",
    )
    pchain.add_argument(
        "--radius", type=int, default=3, help="Default radius when --radii is not given"
    )
    pchain.add_argument("--kernel", default="ball")
    pchain.set_defaults(func=handle_chain)

    psharp = sub.add_parser(
        "sharp-regions",
        help="Detect pointy/outlier regions of a label (distance-based)",
        parents=[parent_parser],
    )
    _add_label(psharp)
    psharp.add_argument(
        "--gauss-sigma", type=float, default=2.0, help="Gaussian sigma for the smooth reference"
    )
    ...
    pcid = sub.add_parser(
        "cc-identify",
        aliases=["identify"],
        help="Count a label's connected components + per-region volume",
        parents=[parent_parser],
    )
    _add_label(pcid)
    ...
    pcex = sub.add_parser(
        "cc-extract",
        help="Save a label's distinct connected components as a new image",
        parents=[parent_parser],
    )
    _add_label(pcex)
    pcex.add_argument(
        ...
        type=int,
        default=None,
        help="Drop components smaller than this many voxels",
    )
    ...
    pcrp = sub.add_parser(
        "cc-regionprops",
        aliases=["regionprops"],
        help="Report region shape stats for a label's components",
        parents=[parent_parser],
    )
    _add_label(pcrp)
    ...
    pshv = sub.add_parser(
        "show-voxels",
        help="Report voxel indices / world coords / bbox for a label",
        parents=[parent_parser],
    )
    _add_label(pshv)
...
def handle_morph_label(args):  # WORKFLOW: delegates to core/segmentation
    out = core_seg.morph_label(
        _load(args.input), args.label, args.operation, args.radius, args.kernel
    )
...
def handle_merge_labels(args):  # WORKFLOW: binarise each label then merge
    out = _load(args.input)
    label_images = [
        core_label.extract_single_label(out, label, binarise=True) for label in args.label
    ]
...
def handle_swap(args):  # THIN+: parse old/new labels (+optional range), one core call
    ...
    if len(old_labels) != len(new_labels):
        logger.error(
            "old labels (%d) and new labels (%d) must match in count.",
            len(old_labels),
            len(new_labels),
        )
        return 1
...
def handle_chain(args):  # WORKFLOW: in-memory chain of morph-label ops
    ...
    if len(args.labels) != n:
        logger.error(
            "--labels (%d) and --operations (%d) must have equal length.", len(args.labels), n
        )
        return 1
    ...
    if len(radii) != n:
        ...
        return 1
    out = core_seg.morph_label_chain(
        _load(args.input), args.labels, args.operations, radii, kernel=args.kernel
    )
    return _write_output(out, args, "chained")


def handle_sharp_regions(args):  # WORKFLOW: distance-based outlier (pointy bits) detection
    out = core_label.distance_based_outlier_detection(
        _load(args.input), args.label, gauss_sigma=args.gauss_sigma
    )
