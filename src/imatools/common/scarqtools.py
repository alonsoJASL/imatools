...
# External tools (MIRTK/CEMRG) can emit this benign error on success.
...
class ScarQuantificationTools:
    ...
    def run_cmd(self, script_dir, cmd_name, arguments, debug=True):
        """Assemble and (unless debug) run an external command via pycemrg.

        Returns ``(status, cmd_str)`` to preserve the legacy contract: callers
        check ``status != 0``.  ``debug=True`` only logs the command (no exec),
        matching the original ``os.system`` behaviour.
        """
        cmd_path = os.path.join(script_dir, cmd_name) if script_dir != "" else cmd_name
        # No-shell execution: each flag/value is its own token.
    ...
    def create_scar_test_image(self, image_size, prism_size, method, origin, spacing, simple):
        im, seg, boundic = itku.generate_scar_image(
            image_size, prism_size, origin, spacing, method, simple
        )
        return im, seg, boundic

    def get_threshold(self, method: int, value, mean_bp, std_bp):
        method_dict = {1: lambda x, mbp, stdb: x * mbp, 2: lambda x, mbp, stdb: x * stdb + mbp}
    ...
    def mask_voxels_above_threshold(
        self, im, mask, thres_mean, thres_std, thres_value=0, mask_value=0, ignore_im=None
    ):
        thres = self.get_threshold(self.get_scar_method(), thres_value, thres_mean, thres_std)
        masked_im = itku.mask_image(
    ...
    def mask_segmentation_above_threshold(
        self, seg_path, im, mask, thres_mean, thres_std, thres_value=0, mask_value=0, ignore_im=None
    ):
