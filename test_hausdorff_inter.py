...
inputParser.add_argument("out_dir", metavar="path", type=str, help="Output directory")
inputParser.add_argument("out_name", metavar="file_name", nargs='?', default="output", type=str, help="Output csv name (saved on out_dir)")
...
for comp in inter_cases:
    ...
    l0=searchFileByType(fullfile(baseDir, u0, '03_completed', patient), 'clean', 'vtk')
    l1=searchFileByType(fullfile(baseDir, u1, '03_completed', patient), 'clean', 'vtk')
    ...
    for l in labels:
        ...
        if save_meshes:
            writeVtk(hdl.GetOutput(), fullfile(out_dir, patient), "thresholded_{}".format(l))
