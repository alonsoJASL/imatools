...
## OPEN DECISIONS — branch & repo management (Jose, 2026-07-15)
...
**Current topology:** `main` (GitHub default, == `development`) · `development` (integration)
· `refactor` (spent) · `legacy` (frozen 0.1.x oracle). Tags: `v0.2.0`, `v0.1.x-final`.
Worktrees: `main/`, `development/`, `refactor/`, `master/` (dir name; holds branch `legacy`).

1. **Delete `origin/refactor`?** It is spent — an ancestor of `main` with nothing unique. Its
...
2. ~~**Is `development` still earning its keep?**~~ **DECIDED (Jose, 2026-07-15): KEEP it as a
   ...
   `feature/x → development → (release) → main + tag`. **This is a commitment, not a
   description** — until now every change went `development` → fast-forward `main` immediately,
...
3. **Branch protection on `main`.** Nothing is enforced today — `main` is force-pushable and
   ...
   - *The PR question:* requiring PRs with **1 approval blocks a solo maintainer** (you cannot
     approve your own). Options are (a) no PR requirement — fast, CI catches breakage after the
     fact; or (b) **PR required with 0 approvals** — costs ~30s per release and makes the 496-test
...
4. ~~**CONTRIBUTING.md — not written.**~~ **DONE (2026-07-15).** Written for **public
   contributors** (Jose's call), so the setup is generic `pip install -e ".[dev]"`. The conda
   envs / `PYTHONPATH`-not-editable-install / worktree layout stay in `CLAUDE.md` — they are
   the maintainer's machine, not a contributor's. Covers: the golden-master rule (given the
   most prominent placement — never re-capture to pass a test; `legacy`/`v0.1.x-final` is the
   ...
   or `CLAUDE.md` — those live in this hub and are **not in the repo**, so contributors cannot
...
5. **Ruff: advisory or blocking?** Currently `continue-on-error: true` in `tests.yml` with a
   14-finding baseline (12 × `E402` in the two parked scripts, 2 × `N802` for `readVtk`/`writeVtk`
   ...
   finding — a PR adding 50 new ones passes silently. The documented gate has always been "0 NEW
   ruff", which the flag cannot express. Closing it is config-only: two `per-file-ignores` for
   `E402`, two `noqa` comments → 0 → drop the flag.

6. **Node 20 deprecation.** Both workflows were bumped (`checkout@v7`, `setup-python@v6`,
   `upload-pages-artifact@v5`, `deploy-pages@v5`) — no action, recorded so the next warning is
   recognisable.
