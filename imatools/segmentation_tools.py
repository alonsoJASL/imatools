...
def extract_single_label(image, label, binarise=False):
    ...
    label_array[image_array == label] = 1 if binarise else label
...
def convert_to_inr(image, out_path):
    ...
    with open(out_path, "wb") as file:
        file.write(header.encode())  # Write header as bytes
...
def main(args) : 
    ...
    if args.mode == "extract":
        if args.label == -1:
            ...
            label_image = extract_single_label(input_image, args.label, args.binarise)
            save_image(label_image, base_dir, f'{outname}_label_{args.label}.nii')

    elif args.mode == "merge":
        merge_labels = args.merge_labels
        merge_labels_str = [str(x) for x in merge_labels]
        if merge_labels == -1:
...
if __name__ == "__main__":
    ...
    input_parser.add_argument("--label", "-l", type=int, default=-1, help="The label to extract. Default: -1 (all labels)")
